// update-all-images-bing.cjs
// Update all articles without images using Bing Image Search
...
async function main() {
  console.log('=== Updating all articles with Bing images ===');
  
  const data = await fetch(SUPABASE_URL + '/rest/v1/articles?select=id,title,category,image');
  ...
  for (const article of articlesWithoutImages) {
    ...
    // Limit to avoid too long execution
    if (count >= 20) break;
  }
